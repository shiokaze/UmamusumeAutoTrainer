  ...
  <div id="create-task-list-modal" class="modal fade">
    <div class="modal-dialog modal-dialog-centered modal-xl">
      <div class="modal-content">
        <h5 class="modal-header">新建任务</h5>
        <div class="modal-body">
          <form>
            <div class="form-group">
              <label for="selectTaskType">⭐ 任务选择</label>
              <select
                v-model="selectedUmamusumeTaskType"
                class="form-control"
                id="selectTaskType"
              >
                <option v-for="task in umamusumeTaskTypeList" :value="task">
                  {{ task.name }}
                </option>
              </select>
            </div>
            <div class="form-group">
              ...
              <select
                v-model="selectedExecuteMode"
                class="form-control"
                id="selectExecuteMode"
              >
            ...
            <div class="row">
              ...
              <div class="col">
                <div class="form-group">
                  <label for="selectAutoRecoverTP"
                    >TP不足时自动恢复（仅使用药水）</label
                  >
                  <select
                    v-model="recoverTP"
                    class="form-control"
                    id="selectAutoRecoverTP"
                  >
            ...
            <div class="row">
              <div class="col-8">
                <div class="form-group">
                  ...
                  <div class="form-inline">
                    <select
                      v-model="presetsUse"
                      style="text-overflow: ellipsis; width: 40em"
                      class="form-control"
                      id="use_presets"
                    >
                      <option v-for="set in cultivatePresets" :value="set">
                        {{ set.name }}
                      </option>
                    </select>
                    <span class="btn auto-btn ml-2" v-on:click="applyPresetRace"
                      >应用</span
                    >
                  </div>
                </div>
              ...
              <div class="col-4">
                <div class="form-group">
                  ...
                  <div class="form-inline">
                    <input
                      v-model="presetNameEdit"
                      type="text"
                      class="form-control"
                      id="presetNameEditInput"
                      placeholder="预设名称"
                    />
                    <span class="btn auto-btn ml-2" v-on:click="addPresets"
                      >保存</span
                    >
                  </div>
                </div>
            ...
            <div class="row">
              <div class="col-4">
                <div class="form-group">
                  <label>⭐ 借用支援卡选择</label>
                  <select
                    v-model="selectedSupportCard"
                    class="form-control"
                    id="selectedSupportCard"
                  >
                    <option
                      v-for="card in umausumeSupportCardList"
                      :value="card"
                    >
                      ({{ card.desc }}) {{ card.name }}
                    </option>
              ...
              <div class="col-2">
                <div class="form-group">
                  ...
                  <input
                    v-model="supportCardLevel"
                    type="number"
                    class="form-control"
                    id="selectSupportCardLevel"
                    placeholder=""
                  />
                </div>
              </div>
              <div class="col-3">
                <div class="form-group">
                  ...
                  <input
                    v-model="clockUseLimit"
                    type="number"
                    class="form-control"
                    id="inputClockUseLimit"
                    placeholder=""
                  />
                </div>
              </div>
            ...
            <div class="row">
              ...
              <div class="col">
                <div class="form-group">
                  ...
                  <input
                    type="number"
                    v-model="expectStaminaValue"
                    class="form-control"
                    id="stamina-value-input"
                  />
                </div>
              </div>
              <div class="col">
              ...
              <div class="col">
                <div class="form-group">
                  ...
                  <input
                    type="number"
                    v-model="expectIntelligenceValue"
                    class="form-control"
            ...
            <div>
              <div class="form-group">
                <span
                  ...
                  v-on:click="switchAdvanceOption"
                  >展开高级选项</span
                >
                <span
                  v-if="showAdvanceOption"
            ...
            <div v-if="showAdvanceOption">
              ...
              <p>
                调整ai对训练的倾向, 不影响最终目标属性,
                一般用于提前完成某一种训练的目标属性，建议权重范围 [-1.0 ~ 1.0],
                0即为不使用额外权重;
              </p>
              <p>
                支援卡或种马强度低时,
                建议增加在一个属性权重的同时减少其他属性同样数值的权重
              </p>
                  ...
                  <div class="form-check">
                    <div v-for="race in umamusumeRaceList_1">
                      ...
                      /><label
                        ...
                        v-if="
                          race.type === 'GI' ||
                          (race.type === 'GII' && !this.hideG2) ||
                          (race.type === 'GIII' && !this.hideG3)
                        "
                      >
                        <span v-if="race.type === 'GIII'"
                          >&nbsp;<span
                            style="background-color: #58c471"
                            class="badge badge-pill badge-secondary"
                            >{{ race.type }}</span
                          >&nbsp;</span
                        >
                        <span v-if="race.type === 'GII'"
                          >&nbsp;<span
                            style="background-color: #f75a86"
                            class="badge badge-pill badge-secondary"
                            >{{ race.type }}</span
                          >&nbsp;</span
                        >
                        <span v-if="race.type === 'GI'"
                          >&nbsp;<span
                            style="background-color: #3485e3"
                            class="badge badge-pill badge-secondary"
                            >{{ race.type }}</span
                          >&nbsp;</span
                        >{{ race.date }} {{ race.name }}</label
                      >
                    </div>
                  </div>
                </div>
            ...
            <div
              v-for="(item, index) in skillLearnPriorityList"
              :key="item.priority"
            >
              <div class="form-group row">
                <label class="col-sm-3" for="'skill-learn-' + item.id"
                  >❗ 学习优先级 {{ item.priority + 1 }}</label
                >
                <div class="col-sm-6">
                  <textarea
                    type="text"
                    v-model="item.skills"
                    class="form-control"
                    id="skill-learn-priority"
                    placeholder="技能1名称,技能2名称,....(使用英文逗号)"
                  ></textarea>
                </div>
                <div class="col-sm-3">
                  <span
                    class="red-button auto-btn ml-2"
                    v-on:click="deleteBox(item, index)"
                    >删除当前优先级</span
                  >
                </div>
              </div>
            </div>
            <span class="btn auto-btn ml-2" v-on:click="addBox(item)"
              >新增优先级</span
            >
            <div class="form-group mb-0">
              <div class="row">
            ...
            <div class="form-group mb-0">
              <div class="row">
                <div class="col">
                  <div class="form-group">
                    <label for="skill-learn-blacklist"
                      >⛔ 黑名单(任何情况下都不学习这些技能)</label
                    >
                    <textarea
                      type="text"
                      v-model="skillLearnBlacklist"
                      class="form-control"
                      id="skill-learn-blacklist"
                      placeholder="钢铁意志,迅疾如风,...(真不会有人点这些吧)"
                    ></textarea>
                  </div>
                </div>
              </div>
            ...
            <div class="form-group">
              <div class="row">
                <div class="col-3">
                  <div class="form-group">
                    <label for="learnSkillOnlyUserProvidedSelector"
                      >育成中仅允许学习上面的技能</label
                    >
                    <select
                      v-model="learnSkillOnlyUserProvided"
                      class="form-control"
                ...
                <div class="col-3">
                  <div class="form-group">
                    <label for="learnSkillBeforeRaceSelector"
                      >在参赛前学习技能</label
                    >
                    <select
                      disabled
                      v-model="learnSkillBeforeRace"
                      class="form-control"
                      id="learnSkillBeforeRace"
                    >
                      <option :value="true">是</option>
                      <option :value="false">否</option>
                    </select>
                ...
                <div class="col-3">
                  <div class="form-group">
                    <label for="inputSkillLearnThresholdLimit"
                      >育成中pt超过此值后学习技能</label
                    >
                    <input
                      v-model="learnSkillThreshold"
                      type="number"
                      class="form-control"
                      id="inputSkillLearnThresholdLimit"
                      placeholder=""
                    />
                  </div>
                </div>
      ...
      <div class="position-fixed" style="z-index: 5; right: 40%; width: 300px">
        <div
          ...
          data-delay="2000"
        >
          <div class="toast-body">✔ 预设保存成功</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      umausumeSupportCardList: [
		...
		{ id: 32, name: "要受人喜爱啊”", desc: "力小栗帽" },
		{ id: 33, name: "见习魔女与漫漫长夜", desc: "速东商变革" },
      ],
      umamusumeRaceList_1: [
      ...
      cultivateDefaultPresets: [
        ...
        },
        {
          ...
          race_list: [
            1601, 1701, 1902, 2103, 2302, 2401, 2701, 2905, 3103, 3303, 3404,
            3601, 4102, 4203, 4408, 4506, 4607, 4804, 4902, 5208, 5407, 5601,
            5709, 5904, 6006, 6602, 6701, 6807, 7007, 7111, 7204,
          ],
          skill: "大胃王",
  ...
  methods: {
    ...
    addTask: function () {
      ...
      for (let i = 0; i < this.skillPriorityNum; i++) {
        if (String(this.skillLearnPriorityList[i].skills) != "") {
          learn_skill_list.push(
            String(this.skillLearnPriorityList[i].skills)
              .split(",")
              .map((item) => item.trim()),
          );
        }
      ...
      var learn_skill_blacklist = this.skillLearnBlacklist
        ? this.skillLearnBlacklist.split(",").map((item) => item.trim())
        : [];
      let payload = {
        app_name: "umamusume",
        ...
        attachment_data: {
          expect_attribute: [
            this.expectSpeedValue,
            this.expectStaminaValue,
            this.expectPowerValue,
            this.expectWillValue,
            this.expectIntelligenceValue,
          ],
          ...
          tactic_list: [
            this.selectedRaceTactic1,
            this.selectedRaceTactic2,
            this.selectedRaceTactic3,
          ],
    ...
    applyPresetRace: function () {
      ...
      if (
        "extraWeight" in this.presetsUse &&
        this.presetsUse.extraWeight != []
      ) {
      ...
      if ("skill" in this.presetsUse && this.presetsUse.skill != "") {
        ...
        for (let i = 0; i < this.presetsUse.skill_priority_list.length; i++) {
          ...
          this.skillLearnPriorityList[i].skills =
            this.presetsUse.skill_priority_list[i];
        }
        while (
    ...
    getPresets: function () {
      this.axios.post("/umamusume/get-presets", "").then((res) => {
        let tmplist = [];
    ...
    addPresets: function () {
      ...
      for (let i = 0; i < this.skillPriorityNum; i++) {
        if (this.skillLearnPriorityList[i].skills != "") {
          preset.skill_priority_list.push([
            this.skillLearnPriorityList[i].skills,
          ]);
        }
      }
      ...
      console.log(JSON.stringify(payload));
      this.axios
        .post("/umamusume/add-presets", JSON.stringify(payload))
        .then(() => {
